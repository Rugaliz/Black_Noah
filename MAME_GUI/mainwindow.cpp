...
//chosing files and saving file path (PC-98)
{
    QString filename_PC98_floppy1 = QFileDialog::getOpenFileName(
                ...
                tr("Chose Game"),
                "/home",
                "Floppy disk image (*.fdi);;MS-DOS disk formats (*.hdm);;All files (*.*)"  // determines types of files and name to display in window
                );
...
void MainWindow::on_Launcher_Button_clicked()
{
        ...
        std::string command = machine_cmd + region + PS_memcard1 + PS_memcard2 + load_cd + "\"" + file1locale + "\""; // pass "'" before and after file path to send a proper quoted path to console
...
void MainWindow::on_Launcher_Button_PC_Engine_clicked()
{
    ...
    std::string command = machine_cmd + load_HuCard + "\"" + HuCard_locale + "\"" + load_cd + "\""+ CD_locale + "\"";    // pass "'" before and after file path to send a proper quoted path to console
