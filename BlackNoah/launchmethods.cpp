...
void LaunchMethods::Nintendo_NES (string region_NES, string ROM_path_NES, string vertical_strech, string glsl_shader) {
    string machine_cmd = "mame ";                      // run machine command linux version
    string file1locale = ROM_path_NES;                 // Path pointed in "ROM1"
    string option = vertical_strech + glsl_shader;
    string command = machine_cmd + region_NES + load_Cartridge + "\"" + file1locale + "\"" + option;    // pass "'" before and after file path to send a proper quoted path to console
