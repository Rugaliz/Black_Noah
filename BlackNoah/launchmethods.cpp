...
std::string load_cassete = (" -cass ");                         // tell MAME to load cassete file into slot
std::string load_Cartridge = (" -cartridge ");                  // tell mame to load files onto cartridge slot
std::string load_Cartridge1 = (" -cart1 ");                     // tell mame to load files onto cartridge slot
std::string load_Cartridge2 = (" -cart2 ");                     // tell mame to load files onto cartridge slot
std::string load_floppy = (" -flop ");                          // tell mame to load files onto floppy tray
std::string load_floppy1 = (" -flop1 ");                        // tell mame to load files onto floppy tray 1
std::string load_floppy2 = (" -flop2 ");                        // tell mame to load files onto floppy tray 2
std::string load_floppy3 = (" -flop3 ");                        // tell mame to load files onto floppy tray 3
std::string load_floppy4 = (" -flop4 ");                        // tell mame to load files onto floppy tray 4
...
void LaunchMethods::PC88 (std::string ROM_path_PC88_floppy1, std::string ROM_path_PC88_floppy2, std::string ROM_path_PC88_Cassete,  std::string vertical_strech, std::string glsl_shader) {
  ...
  std::string option = vertical_strech + glsl_shader;
  std::string command = machine_cmd + load_floppy1 + "\""+ floppy1_locale + "\"" + load_floppy2 + "\"" + floppy2_locale + "\"" + load_cassete + "\"" + ROM_path_PC88_Cassete + "\"" + option;    // pass "'" before and after file path to send a proper quoted path to console
...
void LaunchMethods::PC98 (std::string ROM_path_PC98_HDD, std::string ROM_path_PC98_CDROM, std::string ROM_path_PC98_floppy1, std::string ROM_path_PC98_floppy2, std::string vertical_strech, std::string glsl_shader) {
    ...
    std::string option = vertical_strech + glsl_shader;
    std::string command = machine_cmd + sound_card + load_floppy1 + "\"" + floppy1_locale + "\"" + load_floppy2 + "\""+ floppy2_locale + "\"" + load_cd + "\""+ CD_locale + "\"" + load_hdd + "\""+ HDD_locale + "\"" + option;    // pass "'" before and after file path to send a proper quoted path to console
...
void LaunchMethods::FMMarty (std::string ROM_path_FMMarty_CDROM, std::string ROM_path_FMMarty_floppy, std::string vertical_strech, std::string glsl_shader) {
    ...
    std::string option = vertical_strech + glsl_shader;
    std::string command = machine_cmd  + load_floppy1 + "\"" + floppy1_locale + "\"" + load_cd + "\"" + CD_locale + "\"" + option;    // pass "'" before and after file path to send a proper quoted path to console
...
void LaunchMethods::Nintendo_FDS (std::string ROM_path_FDS, std::string vertical_strech, std::string glsl_shader) {
    ...
    std::string option = vertical_strech + glsl_shader;
    std::string command = machine_cmd + load_floppy + "\""+ file1locale + "\"" + option;    // pass "'" before and after file path to send a proper quoted path to console
    const char *NES_command = command.c_str();
    system(NES_command);
}
...
void LaunchMethods::M$_MSX(std::string ROM_path_MSX_Cass, std::string ROM_path_MSX_Cart1, std::string ROM_path_MSX_Cart2, std::string ROM_path_MSX_Floppy, std::string vertical_strech, std::string glsl_shader) {
    std::string machine_cmd = "mame fsa1fx";               // Launch machine in MAME
    std::string option = vertical_strech + glsl_shader;
    std::string command = machine_cmd + load_cassete + "\"" + ROM_path_MSX_Cass + "\"" + load_Cartridge1 + "\"" + ROM_path_MSX_Cart1 + "\"" + load_Cartridge2 + "\"" + ROM_path_MSX_Cart2 + "\"" + load_floppy + "\"" + ROM_path_MSX_Floppy + "\"" + option;    // pass "'" before and after file path to send a proper quoted path to console
